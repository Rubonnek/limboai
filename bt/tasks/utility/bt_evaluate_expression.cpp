...
void BTEvaluateExpression::set_input_vars(const PackedStringArray &p_input_vars) {
...
Error BTEvaluateExpression::parse() {
	ERR_FAIL_COND_V_MSG(expression_string == String(), FAILED, "BTEvaluateExpression: Expression String is not set.");
	String *processed_input_vars_ptr = processed_input_vars.ptrw();
	if (input_include_delta) {
		processed_input_vars_ptr[0] = "delta";
	}
	for (int i = int(input_include_delta); i < input_vars.size(); ++i) {
